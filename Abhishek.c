...
int setunion(char **strA,int x,char **strB,int y,char **output){
       ...
       for(int i=0; i<x; i++){
       	...
       	for(int i=0; i<y; i++){
       	strcpy(output[count],strB[i]);
       			count++;
       	}

       			char **newoutput;
       			newoutput = (char**)malloc(count*sizeof(char*));
       			for(int j=0; j<count; j++)
       				newoutput[j] = (char*)malloc(50*sizeof(char));
       	int size = distinctStr(output,count,newoutput);
       output = newoutput;
       return size;
}
...
int main(){
  ...
  FILE *outptr_int = fopen("week15-AL4-intersection.txt","w");
