///submitted by DEEPAK PRASAD(200108016)///
#include<stdio.h>
#include <string.h>
#include<stdlib.h>
...
int main() {
    ...
    for( i = 0; i < n; i++) {
        ...
        ///extract  all the fields in reverse order///
        char *ptr = strtok(details, delim_str);
        for ( j = 0; j < 100; ++j) {
            if (ptr[j] == '\0') {
                member[i].course_name[j] = '\0'; ///extract course name///
                break;
            }
            member[i].course_name[j] = ptr[j];
        }
       strrev(member[i].course_name);

        ptr = strtok(NULL, delim_str);
        for ( j = 0; j < 6; ++j) {
            if (ptr[j] == '\0') {
                member[i].course_id[j] = '\0';  ///extract course id///
                break;
            }
            member[i].course_id[j] = ptr[j];
        }
           strrev(member[i].course_id);


        ptr = strtok(NULL, delim_str);
        for ( j = 0; j < 100; ++j) {
            if (ptr[j] == '\0') {
                member[i].name[j] = '\0';   ///extract student name///
                break;
            }
            member[i].name[j] = ptr[j];
        }
         strrev(member[i].name);

        ptr = strtok(NULL, delim_str);
        strrev(ptr);
         member[i].r_no = atoi(ptr);    ///extract student roll no///


        ptr = strtok(NULL, delim_str);
        strrev(ptr);
        member[i].sr_no = atoi(ptr);   ///extract serial no///
